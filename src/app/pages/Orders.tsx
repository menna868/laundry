    ...
    <Link href={`/track-order/${order.id}`} className="block">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md active:scale-[0.99] transition-all">
        ...
        <div className="px-5 py-4 border-b border-gray-50">
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1 min-w-0">
              <p className="text-gray-900 text-sm font-medium truncate">{order.laundryName}</p>
          ...
          <div className="flex items-center gap-3 text-xs text-gray-500">
            <div className="flex items-center gap-1">
              ...
              <span>{order.itemCount} {order.serviceUnit}</span>
            </div>
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="truncate max-w-[80px]">{order.pickupAddress.split(',')[0]}</span>
            </div>
          </div>
        </div>
        ...
        {isActive && (
          <div className="px-5 py-3 bg-gray-50 border-b border-gray-100">
            <div className="flex justify-between items-center mb-1.5">
              ...
              <span className="text-xs font-medium" style={{ color: cfg.color }}>{prog}%</span>
            </div>
        ...
        <div className="px-5 py-3.5 flex items-center justify-between">
          <span className="text-[#1D6076] font-semibold text-base">{order.total} EGP</span>
          <div className="flex items-center gap-2">
            ...
            {isDelivered && order.rating !== null && (
              <div className="flex items-center gap-0.5">
                {[1,2,3,4,5].map(i => (
                  <Star key={i} size={11}
                    className={i <= order.rating! ? 'text-amber-400 fill-amber-400' : 'text-gray-200'}
                    strokeWidth={1}
                  />
                ))}
              </div>
