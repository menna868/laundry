...
function NadeefLogo({ compact = false }: { compact?: boolean }) {
  return (
    <Link href="/" className="flex items-center gap-2 group shrink-0">
      <div className="w-9 h-9 rounded-xl bg-linear-to-br from-[#1D6076] to-[#2a7a94] flex items-center justify-center shadow-sm group-hover:shadow-md transition-shadow">
...
export function TopNav() {
  ...
  const authHref = (path: "/login" | "/signup") =>
    `${path}?from=${encodeURIComponent(pathname || "/")}`;

  const isActive = (href: string) =>
    pathname === href || pathname.startsWith(href);
  const initials =
    `${user?.firstName?.[0] ?? ""}${user?.lastName?.[0] ?? ""}`.trim() || "N";
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-white/60 bg-white/85 shadow-[0_10px_30px_rgba(15,23,42,0.08)] backdrop-blur-xl">
            ...
            {visibleNavLinks.map(({ href, label }) => (
              <Link
                ...
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                  isActive(href)
                    ? "bg-[#1D6076]/10 text-[#1D6076] shadow-sm"
            ...
            {isLoggedIn && user ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 transition-all duration-200 hover:bg-gray-100 hover:shadow-sm"
                >
                  <div className="w-7 h-7 rounded-full bg-linear-to-br from-[#1D6076] to-[#2a7a94] flex items-center justify-center">
                  ...
                  <ChevronDown
                    size={14}
                    className={`text-gray-500 transition-transform ${userMenu ? "rotate-180" : ""}`}
                    strokeWidth={2}
                  />
                </button>
        ...
        {mobileOpen && (
          <div className="fixed inset-0 z-100">
            ...
            <motion.div
              ...
              className="absolute top-0 right-0 bottom-0 flex w-72 flex-col bg-white shadow-2xl"
            >
              ...
              {isLoggedIn && user && (
                <div className="px-5 py-4 bg-[#1D6076]/5 border-b border-gray-100">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-linear-to-br from-[#1D6076] to-[#2a7a94] flex items-center justify-center">
